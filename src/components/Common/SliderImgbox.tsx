...
const SliderImgBox = ({ images }: Props) => {
  ...
  return width > 480 ? (
    <div className="pujadetails-carousel">
      ...
      {images && images?.length && <div className="pujadetails-carousel-dots">
        {images?.map((_, index) => (
          <span
            key={index}
    ...
    <div className="mob-slider">
      <div className="mobile-width-img-container">
        <img
          className="mobile-width-img"
          src={images[0]}
          alt={"image-box"}
          onClick={() => {
            setImgZoomed(images[0]);
            setIsZoomed(true);
          }}
        />
      </div>
