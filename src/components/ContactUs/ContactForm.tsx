...
function ContactForm({ request, toast, redux, fromMessage }: Props) {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      const eventbtn = button_event(
        ...
        "Submit",
        {
          additionalData: { ...formData, fromMessage },
        }
      );
      save_event(redux?.auth?.authToken, location ?? "Home", [eventbtn]);
      request
        ?.POST(contact_contactUs, { ...formData, fromMessage })
        .then((res: any) => {
          pushToDataLayerWithoutEvent({
            event: "contact_form_submission",
            // user_id: redux?.auth?.authToken,
            mobile: redux?.auth?.mobile ?? formData?.phoneNumber,
          ...
          if (formData?.queryType == "Post-Puja Experience / Feedback") {
            pushToDataLayerWithoutEvent({
              event: "feedback_form_submission",
              // user_id: redux?.auth?.authToken,
              mobile: redux?.auth?.mobile ?? formData?.phoneNumber,
              name: formData?.fullName,
              phoneNumber: formData?.phoneNumber,
